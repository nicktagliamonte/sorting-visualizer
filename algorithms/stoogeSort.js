...
function* stoogeSortHelper(arr, start, end) {
  ...
  yield { array: arr.slice(), highlights: currentSubarrayHighlights };
  ...
  if (length >= 3) {
    ...
    for (let i = start; i <= start + 2*third; i++) {
      firstTwoThirdsHighlights.push(i);
    }
    yield { array: arr.slice(), highlights: firstTwoThirdsHighlights };
    ...
    for (let i = start + third; i <= end; i++) {
      lastTwoThirdsHighlights.push(i);
    }
    yield { array: arr.slice(), highlights: lastTwoThirdsHighlights };
